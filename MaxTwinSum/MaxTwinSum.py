...
def pairSum(head: ListNode) -> int:
    # 1) Find midpoint: slow at start of 2nd half (even n)
    slow = fast = head
    while fast and fast.next:
        slow = slow.next
        fast = fast.next.next

    # 2) Reverse second half in-place: prev <- curr -> nxt
    prev, curr = None, slow
    while curr:
        nxt = curr.next
        curr.next = prev
    ...
    # 3) Twin scan: left from head, right from reversed half
    left, right = head, prev
    max_sum = 0
    while right:  # exactly n/2 steps
